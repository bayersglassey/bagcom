...
then
    BUILD_ALL=true
    PROJECTS="`find "$SITE_INDIR" -mindepth 1 -maxdepth 1 -type d | sed "s@^$SITE_INDIR/@@" | tr '\n' ' '`"
else
    BUILD_ALL=false
    PROJECTS="$@"
fi
...
bagcom_buildfile() {
    ...
    do
        ...
        case "$TYPE" in
            ...
            ;;
            bb*)
                read TYPE BB_NAME BB_FRAMES < <(printf '%s\n' "$TYPE")
                test -n "$BB_FRAMES" || BB_FRAMES="$DEFAULT_BB_FRAMES"
                BB_CFILE="$BB_OUTDIR/$BB_NAME.c"
                BB_WAVFILE="$BB_OUTDIR/$BB_NAME.wav"
                BB_BINFILE="$BB_TMPDIR/$BB_NAME"
                BB_RAWFILE="$BB_TMPDIR/$BB_NAME.raw"
                log "Generating $BB_WAVFILE ($BB_FRAMES sample frames)..."
                log "...generating $BB_CFILE, $BB_BINFILE"
                echo "$BLOCK" | do_with_log bb/compile.sh "$BB_CFILE" "$BB_BINFILE"
                log "...generating $BB_RAWFILE"
                do_with_log "$BB_BINFILE" "$BB_FRAMES" >"$BB_RAWFILE"
                log "...generating $BB_WAVFILE"
                do_with_log sox -t raw -b 8 -r 8000 -c 2 -e unsigned-integer "$BB_RAWFILE" "$BB_WAVFILE"
                log "...OK! Generated: $BB_WAVFILE"
                BLOCK="<pre class=\"block-$TYPE\">`echo "$BLOCK" | htmlescape`</pre>"
                BLOCK="$BLOCK$NL<div><a href=\"$BB_STATIC/$BB_NAME.c\">$BB_NAME.c</a> &rarr; <a href=\"$BB_STATIC/$BB_NAME.wav\">$BB_NAME.wav</a></div>"
